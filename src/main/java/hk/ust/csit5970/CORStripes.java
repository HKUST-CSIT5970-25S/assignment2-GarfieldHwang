    ...
    private static class CORMapper1 extends
        ...
        public void map(LongWritable key, Text value, Context context)
            ...
            for (String word : word_set.keySet()) {
                context.write(new Text(word), new IntWritable(word_set.get(word)));
            }
        }
    }
    ...
    public static class CORStripesCombiner2 extends Reducer<Text, MapWritable, Text, MapWritable> {
        ...
        protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
            ...
            for (MapWritable stripe : values) {
                for (Writable word2 : stripe.keySet()) {
                    IntWritable count = (IntWritable) stripe.get(word2);
                    IntWritable combinedCount = (IntWritable) combinedStripe.get(word2);
                    if (combinedCount == null) {
                        combinedStripe.put(word2, count);
                    } else {
                        combinedStripe.put(word2, new IntWritable(combinedCount.get() + count.get()));
                    }
                }
            }
    ...
    public static class CORStripesReducer2 extends Reducer<Text, MapWritable, PairOfStrings, DoubleWritable> {
        ...
        protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
            ...
            for (MapWritable stripe : values) {
                for (Writable word2 : stripe.keySet()) {
                    IntWritable count = (IntWritable) stripe.get(word2);
                    IntWritable finalCount = (IntWritable) finalStripe.get(word2);
                    if (finalCount == null) {
                        finalStripe.put(word2, count);
                    } else {
                        finalStripe.put(word2, new IntWritable(finalCount.get() + count.get()));
                    }
                }
            }

            for (Writable word2 : finalStripe.keySet()) {
                String word1 = key.toString();
                String word2Str = word2.toString();
                double freq_ab = ((IntWritable) finalStripe.get(word2)).get();
                double freq_a = word_total_map.get(word1);
                double freq_b = word_total_map.get(word2Str);
                double cor = freq_ab / (freq_a * freq_b);
                context.write(new PairOfStrings(word1, word2Str), new DoubleWritable(cor));
            }
        }
    }
